...
class flex_files:
    ...
    def parse_headers(self, path = None, debug = False):
        ...
        self.rate = False # Sampling rate, only meaningful if flg_resample is true.
    ...
    def parse(self, structure, record, strip = False):
        ...
        # for i in structure:
            ...
            # print endian, str(v_length), v_type, length
...
class data_out1(flex_files):
    ...
    def __getitem__(self, key):
        ...
        """
            ...
            #raise ValueError, "Only integer key values accepted"
         ...
         #ary = metaArray(self.parse(self.dataB, self.records[rcd_num])[-1])
        ...
        #             str(A_idx[i][1]) + '\t' + \ntmstp
        #             '%(#)0.3e' % {"#": A_idx[i][2]} + '\t' + \Time
        #             str(A_idx[i][3][3]) + '\t' + \Shape
        #             str(A_idx[i][6]) + '->' + str(A_idx[i][7]) \ Range
        #             
        # 
        #desc += self.showIndex()
        #return ary
        ...
        # Parse the record, and only the last field is needed
        data = self.parse([entry_info[3]], self.records[entry_info[4]])[0]
    ...
    def _index_A_(self):
        ...
        while i < (nrcd + 3):
            ...
            if ipart == 1:
                ...
                # No A1 and data in B1 format
                # B_len = irange * jrange * krange
                ...
                while B_len > 0:
                    ...
                    B_len -= records.item_len(i)
                    B_parts += 1
                
                if B_len != 0:
                    # Somthing is wrong, the sum of all B records should
                    # have the same length as the array.
                    raise ValueError, "Residual B record length incorrect, the \
                                    total length of B records parts should be \
                                    the same as the product of array ranges. \
                                    Current Fortran record number: " + str(i) + \
                                    "irange: " + str(irange) + \
                                    "jrange: " + str(jrange) + \
                                    "krange: " + str(krange) + \
                                    "Residual B record length: " + str(Blen)
            else:
                raise ValueError, "ipart value in A record can only be 1 or 0, \
                                got " + str(ipart) + "instead, current Fortran \
...
class pout_hist(flex_files):
    ...
    def __init__(self, path = None, debug = False):
        ...
        self.flg_resample = False
        self.rate = False           # (re)Sampling rate, only meaningful if flg_resample is true.
        # self.flg_resample_immune = False # Immune to resampling, internal use only
