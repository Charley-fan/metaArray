...
class metaArray:
    ...
    """
    def __init__(self, data, info=None, dtype=None, copy=False, debug=False):
        ...
        if info is not None:
            self_info = info
            if debug:
                print "*** Info has been supplied"
        # Otherwise, use data's info attribute if it exists
        elif hasattr(data, 'info'):
            if debug:
                print "*** Orginal object has info attribute"
            self_info = data.copy_info()
        else:
            if debug:
                print "*** No meta info available, generating the defaults"
        ...
        if self_info.has_key('range'):
            if debug:
                print "*** Range description is given"
            self_range = self_info['range'] = range_info(self_info['range'])
        else:
            # Range is the corresponding xyz index
            if debug:
                print "*** No range description available, generating defaults"
            
            self_range = self_info['range'] = range_info()
            self_range['begin'] = list(zeros(ndim, dtype=float))    # Beginning coordinates
            self_range['end'] = list(array(shape, dtype=float))     # Ending coordinates
            self_range['unit'] = [ None ] * ndim
            self_range['label'] = [ None ] * ndim
            self_range['log'] = [ False ] * ndim
            self_range['fft'] = [ False ] * ndim
        ...
        # Register with the range dict object and update conversion functions
    ...
    def __repr__(self):
        ...
        ext_desc = ''
        for field, value in nfo.iteritems():
    ...
    def gen_x2i(self):
        ...
        lst = []
        for i in range(self.ndim):
            if lg[i] is None or lg[i] is False:
